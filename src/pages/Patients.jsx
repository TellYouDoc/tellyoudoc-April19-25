...
function Patients() {
  ...
  const getFilteredAndSortedPatients = () => {
    ...
    if (searchQuery.trim() !== '') {
      ...
      filteredPatients = filteredPatients.filter(patient =>
        ...
        patient.treatmentStage.toLowerCase().includes(query)
      );
    }
  ...
  const displayedPatients = filteredAndSortedPatients.slice(indexOfFirstRecord, indexOfLastRecord);
      ...
      <main className="patients-main">
        ...
        <div className="patients-stats">
          ...
          <div className="stat-item slide-in-item delay-1">
            <div className="stat-value">{allPatients?.filter(patient => patient.session?.toLowerCase() === "current").length || 0}</div>
          ...
          <div className="stat-item slide-in-item delay-3">
            <div className="stat-value">{allPatients?.filter(patient => patient.session?.toLowerCase() === "current").length || 0}</div>
            <div className="stat-label">Recovery Phase</div>
          </div>
        </div>
        ...
        <div className="patients-table-container" ref={tableRef}>
          <table className="patients-table">
            <thead><tr><th onClick={() => requestSort('name')} style={{ textAlign: 'left', paddingLeft: '20px' }}>
              Patient
              {sortConfig.key === 'name' && (
                <span className={`sort-arrow ${sortConfig.direction}`} />
              )}
            </th>
              <th onClick={() => requestSort('age')} style={{ textAlign: 'center', minWidth: '80px' }}>
                Age
                {sortConfig.key === 'age' && (
                  <span className={`sort-arrow ${sortConfig.direction}`} />
                )}
              </th>
              <th onClick={() => requestSort('medicalConditions')} style={{ textAlign: 'center' }}>
                Medical Conditions
                {sortConfig.key === 'medicalConditions' && (
                  <span className={`sort-arrow ${sortConfig.direction}`} />
                )}
              </th>
              <th onClick={() => requestSort('location')} style={{ textAlign: 'center' }}>
                Location
                {sortConfig.key === 'location' && (
                  <span className={`sort-arrow ${sortConfig.direction}`} />
                )}
              </th>
              <th onClick={() => requestSort('lastAppointment')} style={{ textAlign: 'center' }}>
                Last Visit
                {sortConfig.key === 'lastAppointment' && (
                  <span className={`sort-arrow ${sortConfig.direction}`} />
                )}
              </th>
              <th onClick={() => requestSort('status')} style={{ textAlign: 'center' }}>
                Status
                ...
                <tr key={patient.id} className={`table-row-animated delay-${index % 5}`}>
                  ...
                  <td className="text-center">
                    <div className="location-container">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" className="location-icon">
                        <path d="M8 16s6-5.686 6-10A6 6 0 0 0 2 6c0 4.314 6 10 6 10zm0-7a3 3 0 1 1 0-6 3 3 0 0 1 0 6z" />
                      </svg>
                      <span className="location-text">{patient.location || 'N/A'}</span>
                    </div>
                  </td>
                  <td className="text-center">
                    <span className="appointment-text">{patient.lastAppointment}</span>
                  </td>
                  <td className="text-center">
          ...
          <div className="pagination-container">
            <div className="pagination-controls">
              ...
              <div className="pagination-info">
                Showing {displayedPatients.length === 0 ? 0 : Math.min((currentPage - 1) * entriesPerPage + 1, totalPatients)}-{Math.min(currentPage * entriesPerPage, totalPatients)} of {totalPatients} patients
              </div>
            </div>
            <div className="pagination">
              ...
              <button
                className="pagination-button"
                disabled={currentPage === Math.ceil(totalPatients / entriesPerPage) || totalPatients === 0}
                onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(totalPatients / entriesPerPage)))}
              >
                Next
              </button>
            </div>
